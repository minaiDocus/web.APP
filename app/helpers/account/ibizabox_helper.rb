...
module Account::IbizaboxHelper
  def ibizabox_folder_state(folder)
    if folder.state == 'waiting_selection'
      link_to 'Sélectionner les documents', select_organization_customer_ibizabox_documents_path(@organization, @customer, document_contains: { journal: folder.journal.name }), class: 'btn btn-mini'
    else
  ...
  def ibizabox_state_label(state)
    case state
    when 'processing'
      'badge bg-warning semibold'
    when 'ready'
      'badge bg-success semibold'
    when 'inactive'
      'badge bg-secondary semibold'
    end
  end
end
