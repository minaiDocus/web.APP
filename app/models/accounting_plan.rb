...
class AccountingPlan < ApplicationRecord
  ...
  def need_update?
    return false if (user.uses?(:ibiza) && !user.ibiza.try(:auto_update_accounting_plan?)) || (user.uses?(:my_unisoft) && !user.my_unisoft.try(:is_auto_updating_accounting_plan)) || (user.uses?(:sage_gec) && !user.sage_gec.try(:is_auto_updating_accounting_plan))
