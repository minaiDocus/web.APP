class Organization < ApplicationRecord
  ...
  scope :jedeclare_configured, -> { where.not(jedeclare_user: nil, encrypted_jedeclare_password: nil, jedeclare_account_identifier: nil) }
  ...
  def uses_micro_plus?
    self.created_at.strftime('%Y%m%d') >= '20220201' || ['IDOC', 'JC'].include?(self.code)
  end
  ...
  def can_upload_documents?
    return true  if ['IDOC', 'TEEO', 'MCN', 'MCA', 'MCAC', 'DUP'].include?(self.code) || self.created_at >= 2.days.ago
  ...
  def can_create_customers?
    return false if ['MGV'].include?(self.code)
    return true
  end
