...
class Pack::Report < ApplicationRecord
  ...
  def self.search_failed_delivery(user_ids)
    ...
    report = report.joins(:preseizures).where('pack_report_preseizures.updated_at >= ?', 3.months.ago.to_date).where('pack_report_preseizures.is_delivered_to = "" AND pack_report_preseizures.delivery_message IS NOT NULL AND pack_report_preseizures.delivery_message <> "{}" AND pack_report_preseizures.delivery_message <> "" AND pack_report_preseizures.delivery_message NOT LIKE "%already sent%"').group('pack_reports.id', :delivery_message).select('pack_reports.id as report_id', 'pack_reports.name as name', 'pack_report_preseizures.delivery_message as message', 'count(pack_report_preseizures.id) as preseizures_count', 'MAX(pack_report_preseizures.updated_at) as max_date', 'pack_report_preseizures.id as preseizure_id', 'pack_reports.pack_id as pack_id')
