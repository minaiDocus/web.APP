class StoragesMod::Ftp::FetcherWorker
  ...
  def perform
    if FTPDeliveryConfiguration::IS_ENABLED
      UniqueJobs.for 'FtpFetcher' do
        ...
        # StoragesMod::Ftp::Fetcher.fetch(FTPDeliveryConfiguration::FTP_SERVER, FTPDeliveryConfiguration::FTP_USERNAME, FTPDeliveryConfiguration::FTP_PASSWORD, FTPDeliveryConfiguration::FTP_PATH, FTPDeliveryConfiguration::FTP_PROVIDER)


        StoragesMod::Ftp::FetcherWorker::Launcher.check_folder( FTPDeliveryConfiguration::FTP_SERVER,
  ...
  class Launcher
    def self.check_folder(url, username, password, dir = '/', provider = '')
      ...
      if new_dir.any?
        ...
        StoragesMod::Ftp::Fetcher.delay_for(1.minute, queue: :high).prepare_dir
      else
        p "============ No new documents=================="
      end
    end
