class Subscription::Package
  PACKAGES_LIST = [:ido_classique, :ido_mini, :ido_micro, :ido_nano, :ido_x, :ido_plus_micro].freeze
  OPTIONS_LIST  = [:mail_option, :retriever_option, :pre_assignment_option, :digitize_option].freeze
  PRICES_LIST   = { ido_classique: 10, ido_x: 5, ido_mini: 10, ido_micro: 10, ido_plus_micro: 10, ido_nano: 5, mail_option: 10, retriever_option: 5, retriever_option_reduced: 3, pre_assignment_option: 9, signing_piece: 1, digitize_option: 0}
  ...
  class << self
    ...
    def infos_of(package_or_option)
      case package_or_option
        #packages
        when :ido_classique
          { label: 'Téléchargement + Pré-saisie comptable', name: 'basic_package_subscription', group: "iDo'Classique", tooltip: "vous permet de transférer jusqu'à 100 pièces/mois, mutualisation des quotas au niveau du cabinet.", mark: "Au-delà du quota cabinet cumulé, calcul du dépassement simplifié : 0,25€ ht/facture" }
        when :ido_x
          { label: 'Factur-X + Pré-saisie comptable', name: 'idox_package_subscription', group: "iDo'X", tooltip: "vous permet de convertir les pièces venues de jefacture.com (Factur-X) en écritures comptables !", mark: "Attention, les autres modes d’import de documents (email, upload, appli mobile…) ne sont pas disponibles, seuls les fichiers venant de jefacture.com sont autorisés." }
        when :ido_mini
        ...
        #options
        when :mail_option
          { label: 'Envoi par courrier A/R', name: 'mail_package_subscription', group: "Courrier", tooltip: "vous permet d’adresser vos pièces par courrier à notre centre de numérisation. Disponible pour les forfaits iDo Mini et iDo Classique" }
        when :retriever_option
          { label: 'Récupération banque', name: "retriever_package_subscription", group: "Automates", tooltip: "vous permet de bénéficier des automates de récupération bancaires" }
    ...
    def excess_of(package)
      case package
        ...
        when :ido_plus_micro
          {
            pieces:         { limit: 20, price: 30, per: :month },
            preassignments: { limit: 20, price: 30, per: :month }
          }
        when :ido_nano
          {
            pieces:         { limit: 100, price: 25, per: :year },
