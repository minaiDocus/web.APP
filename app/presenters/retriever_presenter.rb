  ...
  def state(scope=:account)
    if retriever.waiting_selection?
      if retriever.provider? || retriever.provider_and_bank?
        if scope == :account
          h.content_tag :span, 'INFO: En attente de séléction de documents', class: 'badge bg-warning'
        elsif scope == :collaborator
          h.content_tag :span, 'INFO: En attente de séléction de documents', class: 'badge bg-warning'
        elsif scope == :admin
          h.content_tag :span, 'Sélection des documents', class: 'badge bg-secondary'
        end
      else
        if scope == :account
          h.content_tag :span, 'INFO: En attente de séléction de comptes', class: 'badge bg-warning'
        elsif scope == :collaborator
          h.content_tag :span, 'INFO: En attente de séléction de comptes', class: 'badge bg-warning'
        elsif scope == :admin
          h.content_tag :span, 'Sélection des comptes', class: 'badge bg-secondary'
        end
      end
      ...
      h.content_tag :span, 'Suppression en cours', class: 'badge bg-secondary'
    elsif retriever.unavailable?
      content = formatted_state({class: 'badge bg-secondary'})
      #Temporary disable new provider request
      # if scope == :account
      #   content + h.link_to("Demander la création d'un automate", h.new_account_new_provider_request_path, class: 'btn btn-dark')
      # elsif scope == :collaborator
      #   content + h.link_to("Demander la création d'un automate", h.new_account_organization_customer_new_provider_request_path(user.organization, user), class: 'btn btn-dark')
      # elsif scope == :admin
      #   content + h.content_tag(:span, "Demander la création d'un automate", class: 'badge bg-secondary')
      # end
    else
      ...
      formatted_state({class: "badge bg-#{label_type}"})
    end
  end

  def action_link(organization=nil, customer=nil)
    if retriever.ready? or retriever.error?
      h.link_to glyphicon('loop-circular'), '#', class: "trigger_retriever trigger_retriever_#{retriever.id} btn btn-dark", data: { id: retriever.id }, title: "Synchroniser", rel: 'tooltip'
    else
      ''
    end
  end
