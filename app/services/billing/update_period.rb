...
class Billing::UpdatePeriod
  ...
  def execute
    return true if @period.organization #temp fix
  ...
  def base_options
    if @subscription.is_package?('ido_annual')
      ...
      @period.get_active_options.each do |_p_option|
        if ['EXT'].include?(@subscription.user.organization.code)
  ...
  def journals_option
    if ['DI', 'BIN', 'EXTE', 'CRE', 'ACC', 'ACDA', 'GW', 'FOO', 'ADV'].include?(@subscription.user.organization.code)
      additionnal_journals = @subscription.number_of_journals - 5
    else
      additionnal_journals = @subscription.user.account_book_types.where('created_at < ?', @time_end).count - 5
    end
  ...
  def bank_accounts_options
    ...
    if @period.user.code == 'COMP%GPINC'
      excess_bank_accounts = 5
    elsif @period.user.code == 'ADV%GAVELLE'
      excess_bank_accounts = 0
    elsif @period.user.code == 'JC%PRS' || @period.user.code == 'SIDF%ANIMEDIA'
      excess_bank_accounts = 0
    else
      excess_bank_accounts = bank_ids.try(:size).to_i - 2
    end
