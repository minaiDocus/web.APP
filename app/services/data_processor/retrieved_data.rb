...
class DataProcessor::RetrievedData
  ...
  def initialize(retrieved_data, type_synced=nil, user=nil)
    ...
    if @retrieved_data.try(:id).present?
  ...
  def get_bank_account_of(account)
    @user.bank_accounts.where('api_id = ? OR (bank_name = ? AND name = ? AND number = ? AND api_name = "budgea" AND is_used = true)', account['id'], retriever.service_name, account['name'], account['number']).first
  end
