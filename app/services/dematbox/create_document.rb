  ...
  def execute
    CustomUtils.mktmpdir('dematbox_create_document') do |dir|
      ...
      if valid?
        ...
        Notifications::DematboxUploaded.delay_for(1.minutes, queue: :high).notify_dematbox_document_uploaded({ temp_document_id: @temp_document.id, remaining_tries: 3, from: 'delay' }) if Rails.env == 'production' ### WORKARROUND: if the first way (async) failed
      end
    end
  end
