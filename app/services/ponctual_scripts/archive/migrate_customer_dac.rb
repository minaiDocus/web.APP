class PonctualScripts::MigrateCustomerDac < PonctualScripts::PonctualScript
  def self.execute
    #------------------- VERY IMPORTANT : ------------------
  ...
  def execute
    ...
    customers    = User.where(code: ['DAC%IMEMAIA']) #Customers to migrate
  ...
  def backup
    organization = { 'VOL%IMEMAIA' => 149 } #Customers to rollback with their previous organization id
    customers    = User.where(code: ['VOL%IMEMAIA']) #Customers to rollback with their current codes
