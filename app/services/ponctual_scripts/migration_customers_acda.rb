class PonctualScripts::MigrateCustomerAcda < PonctualScripts::PonctualScript
  def self.execute
    #------------------- VERY IMPORTANT : ------------------
    ...
    # update customer code to CustomUtils if needed
    new().run
  end
  ...
  def execute
    organization = Organization.find_by_code 'LA2S' #The new organization
    customers    = User.where(code: ['ACDA%PARUPRO', 'ACDA%PARUNET']) #Customers to migrate
  ...
  def backup
    organization = { 'ACDA%PARUPRO' => 24, 'ACDA%PARUNET' => 24 } #Customers to rollback with their previous organization id
    customers    = User.where(code: ['ACDA%PARUPRO', 'ACDA%PARUNET']) #Customers to rollback with their current codes
  ...
  def models
    [ Pack, Pack::Report, Pack::Piece, Pack::Report::Preseizure, Pack::Report::TempPreseizure, Pack::Report::Expense, Operation, PaperProcess, PeriodDocument, PreAssignmentDelivery, PreAssignmentExport, RemoteFile, TempDocument, TempPack, Order ]
  end
end