...
class Subscription::Evaluate
  ...
  def execute
    ...
    if period.is_active?(:ido_classique) || period.is_active?(:ido_micro) || period.is_active?(:ido_plus_micro) || period.is_active?(:ido_mini) || period.is_active?(:ido_nano)
      authorize_upload
    else
      unauthorize_upload
    end
    ...
    elsif period.is_active?(:ido_micro) || period.is_active?(:ido_plus_micro) || period.is_active?(:ido_nano) || ( (period.is_active?(:retriever_option) || period.is_active?(:digitize_option)) && !( period.is_active?(:ido_classique) || period.is_active?(:ido_mini) ))
      authorize_pre_assignment
    else
...
private
  ...
  def unauthorize_retriever
    if @customer.options.is_retriever_authorized
      ...
      Retriever::Remove.delay(queue: :high).execute(@customer.id.to_s) if @customer.budgea_account.present?
    end
  end
