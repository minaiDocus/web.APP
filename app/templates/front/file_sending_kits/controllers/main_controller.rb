...
class FileSendingKits::MainController < OrganizationController
  ...
  # PUT /organizations/:organization_id/file_sending_kit
  def update
    ...
    if @file_sending_kit.save
      ...
      flash[:error] = errors_to_list @file_sending_kit
    end
        ...
        if without_shipping_address.count == 0 && error_logo.empty?
          ...
          if without_shipping_address.count != 0
            ...
            without_shipping_address.each do |client|
              errors << "</br><a href='#{organization_customer_path(@organization, client)}' target='_blank'>#{client.info}</a>"
            end
          end
          if error_logo.any?
  ...
  # GET /organizations/:organization_id/file_sending_kit/folders
  def folders
    send_pdf('folders.pdf')
  end

  # GET /organizations/:organization_id/file_sending_kit/mails
  def mails
    send_pdf('mails.pdf')
  end

  # GET /organizations/:organization_id/file_sending_kit/customer_labels
  def customer_labels
    send_pdf('customer_labels.pdf')
  end

  # GET /organizations/:organization_id/file_sending_kit/workshop_labels
  def workshop_labels
