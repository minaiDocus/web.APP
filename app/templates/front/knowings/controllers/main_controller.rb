...
class Knowings::MainController < OrganizationController
  before_action :verify_rights
  before_action :verify_existant, only: %w[new create]
  before_action :load_knowings, only: %w[edit update]

  prepend_view_path('app/templates/front/knowings/views')
  ...
  # POST /organizations/:organization_id/knowings
  def create
    ...
    if @knowings.save
      ...
      redirect_to organization_path(@organization, tab: 'knowings')
    else
      render :new
    end
  end
  ...
  # PUT /organizations/:organization_id/knowings
  def update
