...
class MyDocuments::PiecesController < MyDocuments::AbaseController
  ...
  def index_customers
    ...
    @users = accounts.includes(:options, :ibiza, :subscription, organization: [:ibiza, :exact_online, :my_unisoft, :coala, :cogilog, :sage_gec, :acd, :quadratus, :cegid, :cegid_cfe, :csv_descriptor, :fec_agiris]).active.order(code: :asc)
