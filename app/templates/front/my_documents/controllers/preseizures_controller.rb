...
class MyDocuments::PreseizuresController < MyDocuments::AbaseController
  ...
  def show
    if params[:view] == 'by_type'
      if @preseizure.operation
        render file: Rails.root.join('app/templates/front/my_documents/views/documents/operations/_operation_box.html.haml'), locals: { preseizure: @preseizure, animation: 'toLeft' }
      else
        @pieces = [@preseizure.piece]
        render file: Rails.root.join('app/templates/front/my_documents/views/documents/pieces/_piece_box.html.haml')
      end
    else
      render partial: 'show'
    end
