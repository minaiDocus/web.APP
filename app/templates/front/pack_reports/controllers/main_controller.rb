...
class PackReports::MainController < OrganizationController
  ...
  # POST /organizations/:organization_id/pack_reports/deliver
  def deliver
    preseizures = @report.preseizures.by_position.not_locked.not_delivered
    PreAssignment::CreateDelivery.new(preseizures, %w[ibiza exact_online my_unisoft sage_gec acd cegid_cfe]).execute
  ...
  # GET /organizations/:organization_id/pack_reports/select_to_download
  def select_to_download; end

  # POST /organizations/:organization_id/pack_reports/:id/download
  def download
