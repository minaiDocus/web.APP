...
class RetrievedDatasV2::OperationsController < RetrieverController
  ...
  def index
    ...
    render partial: 'operations', locals: { operations: @operations, waiting_operations_count: waiting_operations.count }
  end
  ...
  def unlock_operations
    ...
    if operations(false).present? && params[:banking_operation_contains].present?
      ...
      if count > 0
        json_flash[:success] = "#{count} opération(s) débloquée(s) avec succès."
      else
        json_flash[:error] = 'Aucune opération a été débloquée.'
      end
    else
      json_flash[:error] = 'Veuillez appliquer un filtre'
    end
